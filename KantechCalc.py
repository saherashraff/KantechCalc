...
class CCTVApp:
    def __init__(self, root):
        self.root = root
        self.root.title("CCTV MASTER V31.0 - DETAILED ANALYTICS")
        self.load_all_data()
        self.setup_ui()
    ...
    def setup_ui(self):
        ...
        ttk.Button(self.tabs[2], text="RUN MANUAL CALCULATION", command=lambda: self.run_logic(False)).pack(pady=10)
        ...
        # REMAINING TABS (Simplified setup)
        self.setup_mgt()
    ...
    def run_logic(self, auto):
        ...
        if auto:
            ...
            if active_hw:
                ...
                for r1 in splits:
                    u_list, cur_cams = [], [dict(c) for c in cams]
                    ratios = [r1, 1.0-r1] if num == 2 else [1.0/num]*num if num > 2 else [1.0]
                    valid = True
                    for i, ratio in enumerate(ratios):
                        u_brk, u_mb, u_tb, u_c = {}, 0, 0, 0
                        for c in cur_cams:
        ...
        else: txt.insert("1.0", "ERROR: CONFIG EXCEEDS HARDWARE LIMITS (Check Channels/Mbps/Slots)")
