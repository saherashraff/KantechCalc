...
class KantechDCCalculatorGUI:
    def __init__(self):
        ...
        # Controller models with SMART CARD READER capacity (kt-4 renamed, cost updated)
        self.controllers = [
    ...
    def create_main_tab(self):
        ...
        # Title
        title_label = ttk.Label(tab, text="ACCESS CONTROL SYSTEM CALCULATOR", 
                               style='Title.TLabel')
        ...
        ttk.Button(actions_frame, text="Calculate SWH/GSTAR", 
                  command=self.show_gstar_calculation).pack(side=tk.LEFT, padx=5)
    ...
    def create_door_types_tab(self):
        ...
        columns = ('ID', 'Name', 'Card Reader', 'Smart Card Reader', 'Bio-metric', 'Door Sensor', 'Readers', 'Inputs', 'Outputs')
    ...
    def create_calculation_tab(self):
        ...
        # Create content for Kantech tab
        self.create_kantech_calculation_frame(kantech_calc_tab)
        
        # Create content for SWH tab
    ...
    def create_license_tab(self):
        ...
        # Create content for license tabs
    ...
    def create_kantech_license_frame(self, parent):
        ...
        # Calculate button
        ttk.Button(parent, text="Calculate Kantech License Requirements", 
    ...
    def create_swh_license_frame(self, parent):
        ...
        # Calculate button
        ttk.Button(parent, text="Calculate SWH License Requirements", 
    ...
    def update_overview(self):
        ...
        if self.dc_lines:
            for dc in self.dc_lines:
                ...
                overview += f"Bio-metric: {dc.fingerprint}, Door Sensor: {dc.door_sensor}\n"
        else:
    ...
    def apply_door_type_to_dc(self):
        ...
        new_dc_line = DCDevice(
            ...
            smart_card_reader=door_type.config.smart_card_reader,
    ...
    def select_controllers_no_smart_readers(self, total_normal_readers):
        """Original algorithm for systems without smart card readers (uses kt-4 with new price)"""
        best_solution = None
        best_cost = float('inf')
        
        # Maximum possible of each controller type
        max_kt4 = max(1, total_normal_readers // 4 + 2)
        max_kt2 = max(1, total_normal_readers // 2 + 2)
        max_kt1 = max(1, total_normal_readers + 2)
        
        for kt4 in range(max_kt4 + 1):
            for kt2 in range(max_kt2 + 1):
                for kt1 in range(max_kt1 + 1):
                    # Skip the case where all are 0 - WE MUST HAVE AT LEAST ONE CONTROLLER
                    if kt4 == 0 and kt2 == 0 and kt1 == 0:
                        continue
                        
                    readers_provided = (kt4 * 4 + kt2 * 2 + kt1 * 1)
                    cost = (kt4 * 2395 + kt2 * 750 + kt1 * 450)  # kt-4 price updated
        ...
        if best_solution:
            ...
            inputs_provided = (best_solution['kt-4'] * 16 + 
                             best_solution['kt-2'] * 8 + 
                             best_solution['kt-1'] * 4)
            
            outputs_provided = (best_solution['kt-4'] * 4 + 
    ...
    def select_controllers_with_smart_readers(self, total_normal_readers, total_smart_readers):
        """NEW: Special algorithm for systems WITH smart card readers
           Smart card readers can ONLY use smart card reader slots.
           Biometric (fingerprint) and regular card readers use normal slots.
        """
        best_solution = None
        best_cost = float('inf')
        
        # Maximum possible of each controller type
        max_kt4 = max(1, (total_normal_readers + total_smart_readers) // 4 + 2)
        ...
        for kt4 in range(max_kt4 + 1):
            for kt2 in range(max_kt2 + 1):
                for kt1 in range(max_kt1 + 1):
                    ...
                    normal_capacity = (kt4 * 4 + kt2 * 2 + kt1 * 1)  # normal reader slots
                    smart_capacity = (kt4 * 4 + kt2 * 2 + kt1 * 1)   # smart card reader slots
                    
                    cost = (kt4 * 2395 + kt2 * 750 + kt1 * 450)  # kt-4 price updated
                    
                    # Check if this combination can handle both normal and smart readers
                    # Normal readers must fit in normal slots; smart readers must fit in smart slots
                    if normal_capacity >= total_normal_readers and smart_capacity >= total_smart_readers:
                        if cost < best_cost:
                            best_cost = cost
                            ...
                            best_solution = {
                                ...
                                'cost': cost,
                                'extra_normal_readers': normal_capacity - total_normal_readers,
                                'extra_smart_readers': smart_capacity - total_smart_readers,
                                'normal_readers_used': total_normal_readers,
                                'smart_readers_used': total_smart_readers
                            }
        ...
        return {
            ...
            'normal_readers_used': min(1, total_normal_readers),
            'smart_readers_used': min(1, total_smart_readers),
            'algorithm': 'fallback',
            'inputs_provided': 4,
            'outputs_provided': 2
        }
    ...
    def calculate_all_kantech(self):
        ...
        for dc_line in self.dc_lines:
            ...
            total_kt4 += controller_info['kt-4']
        ...
        for result in all_results:
            ...
            if controllers.get('algorithm') == 'smart_reader':
                result_text += f"  [SMART CARD READER MODE] (Smart readers use dedicated smart slots)\n"
                result_text += f"  Controllers: kt-4({controllers['kt-4']}), kt-2({controllers['kt-2']}), kt-1({controllers['kt-1']})\n"
                ...
                result_text += f"  Allocation: {controllers['smart_readers_used']} smart readers, "
                result_text += f"{controllers['normal_readers_used']} normal readers (card+biometric)\n"
            else:
                result_text += f"  Controllers: kt-4({controllers['kt-4']}), kt-2({controllers['kt-2']}), kt-1({controllers['kt-1']})\n"
        ...
        result_text += f"  kt-4: {total_kt4} units  (${total_kt4 * 2395})\n"
        ...
        if total_smart_readers > 0:
            ...
            result_text += f"  Note: Using smart card reader optimized algorithm (smart only on smart slots)\n"
        result_text += f"Total controller cost: ${total_kt4 * 2395 + total_kt2 * 750 + total_kt1 * 450}\n"
        result_text += f"Total expansion cost:  ${total_expansion_cost}\n"
        result_text += f"GRAND TOTAL:           ${grand_total_cost}\n"
        
        total_controllers = total_kt4 + total_kt2 + total_kt1
        result_text += f"\nLicense Reference:\n"
    ...
    def calculate_selected_kantech(self):
        ...
        if dc_totals['smart_card_readers'] > 0:
            ...
            result_text += "   Smart card readers use dedicated smart slots only.\n"
            result_text += "   Biometric and card readers use normal slots.\n\n"
        
        result_text += "STEP 1: SELECT CONTROLLERS\n"
        result_text += "-" * 40 + "\n"
        result_text += f"Selected Controllers for DC Line {dc_line.dc_number}:\n"
        result_text += f"  kt-4: {controller_info['kt-4']} units\n"
        ...
        if controller_info.get('algorithm') == 'smart_reader':
            ...
            result_text += f"  Smart readers used: {controller_info['smart_readers_used']}\n"
            result_text += f"  Normal readers used: {controller_info['normal_readers_used']}\n"
    ...
    def calculate_kantech_license(self):
        ...
        result_text += f"Total Controllers Needed: {total_controllers}\n"
        result_text += f"Controller Breakdown:\n"
        result_text += f"  kt-4: {total_kt4} units\n"
    ...
    def export_kantech_results(self):
        ...
        if filename:
            try:
                ...
                # Add grand total
                total_kt4 = sum(r['controllers']['kt-4'] for r in self.kantech_all_results)
    ...
    def export_system_summary(self):
        ...
        if filename:
            try:
                ...
                total_smart_card_reader = sum(dc.smart_card_reader for dc in self.dc_lines)
    ...
    def show_gstar_calculation(self):
        ...
        self.notebook.select(3)  # Calculations tab
        calc_notebook = self.notebook.winfo_children()[3].winfo_children()[0]
        calc_notebook.select(1)  # SWH/GSTAR tab
