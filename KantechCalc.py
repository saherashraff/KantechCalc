...
ALL_MODELS = JBOD_LIST + HOLIS_DATA
...
class CCTVApp:
    def __init__(self, root):
        self.root = root
        self.root.title("CCTV MASTER V23.2 - RESTORED LOGIC")
    ...
    def run_logic(self, auto):
        ...
        if auto:
            ...
            # Selection logic: RAID mode only uses RAID list. JBOD mode uses JBOD list.
            search_list = RAID_ONLY if mode in ["RAID 5", "RAID 6"] else JBOD_LIST
            parity = 1 if mode == "RAID 5" else 2 if mode == "RAID 6" else 0
            
            # Check Holis separately for Auto
            full_search = search_list + HOLIS_DATA
            for m in full_search:
                if t_c <= m[2] and t_m <= m[3]:
                    hc, hd = get_best_hdd(t_t, m[4], parity, self.hdd_prices)
                    if hd and (m[5] + hc) < best_cost:
                        best_cost, best_cfg = (m[5] + hc), {"total": (m[5] + hc), "units": [{"m": m, "c": t_c, "mb": t_m, "tb": t_t, "h": hd, "mode": mode}]}
        else:
            active = []
            ...
            if active:
                c1, m1, t1 = t_c, t_m, t_t
                h1_c, h1_d = get_best_hdd(t1, active[0]['hw'][4], active[0]['parity'], self.hdd_prices)
                if h1_d and c1 <= active[0]['hw'][2] and m1 <= active[0]['hw'][3]:
                    best_cost, best_cfg = (active[0]['hw'][5] + h1_c), {"total": (active[0]['hw'][5] + h1_c), "units": [{"m": active[0]['hw'], "c": c1, "mb": m1, "tb": t1, "h": h1_d, "mode": active[0]['mode']}]}

        txt = self.res_txt if auto else self.man_txt
        txt.delete("1.0", tk.END)
        if best_cfg: txt.insert("1.0", self.generate_detailed_report(best_cfg, cams, t_c, t_m, t_t, "AUTO" if auto else "MANUAL"))
